...
public final class ZipService {
    ...
    public void validateZipFile(Path file, TaskListener listener) throws IOException {
        listener.getLogger().println("[EndpointS3SCM] Validating ZIP archive...");

        try (InputStream in = Files.newInputStream(file);
             ZipInputStream zis = new ZipInputStream(in)) {

            if (zis.getNextEntry() == null) {
                throw new AbortException("[EndpointS3SCM] Downloaded file is not a valid ZIP archive or is empty");
            }
        } catch (AbortException e) {
            throw e;
        } catch (Exception e) {
            throw new AbortException("[EndpointS3SCM] Invalid ZIP archive: " + e.getMessage());
        }

        listener.getLogger().println("[EndpointS3SCM] ZIP archive validation passed");
    }
    ...
    public void extractZipSecurely(Path zipFile,
        ...
        if (stripTopLevelDir) {
            ...
            if (topLevelToStrip == null) {
                log.println("[EndpointS3SCM] stripTopLevelDir=true, but no common top-level directory detected. Extracting as-is.");
            } else {
                log.printf("[EndpointS3SCM] Common top-level directory detected: '%s'. It will be stripped.%n", topLevelToStrip);
            }
        }
        ...
        try (InputStream fis = Files.newInputStream(zipFile);
            ...
            while ((entry = zis.getNextEntry()) != null) {
                if (++fileCount > MAX_FILES) {
                    throw new AbortException("[EndpointS3SCM] Too many files in archive (> " + MAX_FILES + "). Possible Zip Bomb.");
                }
                ...
                if (safeName == null) {
                    log.printf("[EndpointS3SCM] SECURITY: Blocked unsafe ZIP entry: %s%n", rawName);
                    zis.closeEntry();
                    continue;
                }
                ...
                if (!isDescendant(workspace, target)) {
                    log.printf("[EndpointS3SCM] SECURITY: Blocked Zip Slip attempt: '%s' → '%s'%n", rawName, finalPath);
                    zis.closeEntry();
                    continue;
                }
                    ...
                    try (OutputStream os = target.write()) {
                        ...
                        while ((len = zis.read(buffer)) > 0) {
                            ...
                            if (fileSize > MAX_FILE_SIZE) {
                                throw new AbortException("[EndpointS3SCM] Single file too large (> " +
                                        (MAX_FILE_SIZE / 1024 / 1024) + " MB): " + finalPath);
                            }
                            if (totalExtracted > MAX_EXTRACTED_SIZE) {
                                throw new AbortException("[EndpointS3SCM] Total extracted data exceeds limit (" +
        ...
        log.printf("[EndpointS3SCM] Extraction completed: %d files, %d bytes total%n",
    ...
    /**
     * Checks if a target file path is within the workspace directory (prevents Zip Slip).
     *
     ...
     * @param topLevelToStrip Common top-level directory to remove (can be null)
    ...
    private String applyTopLevelStrip(String normalized, String topLevelToStrip, PrintStream log) {
        ...
        log.printf("[EndpointS3SCM] Entry '%s' is not under common top-level dir '%s' — keeping original path.%n",
    ...
    private String detectCommonTopLevelDir(Path zipFile, PrintStream log) throws IOException {
        log.println("[EndpointS3SCM] Detecting common top-level directory...");
            ...
            while ((entry = zis.getNextEntry()) != null) {
                ...
                if (slashIdx > 0) {
                    ...
                    if (candidate == null) {
                        ...
                        log.printf("[EndpointS3SCM] Multiple top-level directories found: '%s' and '%s'%n",
                                candidate, firstDir);
                        return null;
                    }
                } else {
        ...
        if (!rootItems.isEmpty()) {
            log.printf("[EndpointS3SCM] Archive contains items in root directory: %s%n", rootItems);
            return null;
        }

        if (candidate != null) {
            log.printf("[EndpointS3SCM] Common top-level directory found: '%s'%n", candidate);
            return candidate;
        }

        log.println("[EndpointS3SCM] No common top-level directory detected");
        return null;
    }
}